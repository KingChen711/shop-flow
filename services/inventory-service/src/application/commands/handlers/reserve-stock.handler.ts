...
export class ReserveStockHandler implements ICommandHandler<ReserveStockCommand> {
  ...
  async execute(command: ReserveStockCommand): Promise<Reservation> {
    const { orderId, productId, quantity, ttlMinutes } = command;
    ...
    return this.lockService.withLock(lockKey, async () => {
      ...
      // Create reservation record first (validates the data)
      const reservation = Reservation.create(orderId, productId, quantity, ttlMinutes);
      ...
      try {
        ...
        this.logger.log(
          `Stock reserved for order ${orderId}, product ${productId}: ${quantity} units`
        );
