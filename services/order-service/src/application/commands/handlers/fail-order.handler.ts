...
import { OrderFailedEvent } from '@domain/events/order.events';
...
export class FailOrderHandler implements ICommandHandler<FailOrderCommand> {
  ...
  async execute(command: FailOrderCommand): Promise<Order> {
    ...
    if (!order.isPending()) {
      throw new ConflictError(`Cannot fail order in status: ${order.status}`);
    }
    ...
    try {
      ...
      // Create event
      const event = new OrderFailedEvent(orderId, order.userId, reason);
      ...
      this.logger.log(`Order failed: ${orderId}, reason: ${reason}`);
